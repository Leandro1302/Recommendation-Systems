...
if not os.path.exist("user2movies.json") or \
    not os.path.exist("movie2user.json") or \
...
for i in range(M):
    ...
    users_i = movie2user[i] # users who voted movie i
    users_i_set = set(users_i)
    
    ratings_i = {user: usermovie2rating[(user,i)] for user in users_i} # movie i rating of user
    avg_i = np.mean(list(ratings_i.values())) # average rating of movie i
    dev_i = {user:(rating - avg_i) for user, rating in ratings_i.items()} # deviation of movie i
    dev_i_values = np.array(list(dev_i.values()))
    sigma_i = np.sqrt(dev_i_values.dot(dev_i_values)) # sigma of movie i
